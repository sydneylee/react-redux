...
class NameForm extends Component {
    ...
    schemaAsync = yup.object().shape({
        fullname: yup.object().shape({
            ...
            //     })
            // }),
            firstname : yup.string().required().min(3).max(5).equalTo(yup.ref('lastname'), 'should be same as lastname'),
            ...
            //     })
            // }),
            // lastname : yup.string().equalTo(yup.ref('fullname.firstname'), 'passwords do not match')//notworking
            ...
            lastname : yup.string().required().min(3).max(5).equalTo(yup.ref('firstname'))
        })
    });
